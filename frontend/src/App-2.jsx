/* eslint-disable no-nested-ternary */
/* eslint-disable import/no-named-as-default-member */
import React, { useState, useEffect } from 'react'
import './App.css'
import io from 'socket.io-client'
import ServerView from './components/ServerView/ServerView'
import UserView from './components/UserView/UserView'
import HomePage from './pages/HomePage/HomePage'

const SERVER = process.env.REACT_APP_ENDPOINT || 'http://localhost:8080'

function App({ initialUserRole }) {
  // init all the used variables
  const [socket, setSocket] = useState(null)
  const [isConnected, setConnected] = useState(false)
  const [users, setUsers] = useState([])
  const [messages, setMessages] = useState([])
  const [hiddenWord, setHiddenWord] = useState()
  const [userDrawer, setDrawer] = useState()

  const [userRole, setUserRole] = useState(initialUserRole)

  const [isRegistered, setRegistered] = useState(false)

  const events = {
    connect: 'connect',
    disconnect: 'disconnect',
    updateUsers: 'update-users',
    newMessage: 'new-message',
    findWord: 'find-word',
    drawerUsers: 'drawer-users',
    retrieveUser: 'retrieve-user',
  }

  const retrieveUser = (oldUser) => {
    socket.emit(events.registration, {
      id: socket.id,
      pseudo: oldUser.pseudo,
      avatar: oldUser.avatar,
    })
    setRegistered(true)
  }

  // initialize the socket IO connectionx
  useEffect(() => {
    // localStorage.clear()
    const id = localStorage.getItem('pentaroom-socket-id')
    const newSocket = id
      ? io(SERVER, { query: `id=${id}` })
      : io(SERVER)
    setSocket(newSocket)
    return () => socket?.close()
  }, [])

  // Socket IO Client Manager
  useEffect(() => {
    if (!socket) return
    socket.on(events.connect, () => {
      localStorage.setItem('pentaroom-socket-id', socket.id)
      setConnected(true)
    })

    socket.on(events.disconnect, () => setConnected(false))
    socket.on(events.findWord, (word) => setHiddenWord(word))
    socket.on(events.updateUsers, (listUsers) => setUsers(listUsers))
    socket.on(events.drawerUsers, (drawer) => setDrawer(drawer))

    socket.on(events.newMessage, (message) => {
      setMessages((oldMessages) => [...oldMessages, {
        ...message,
        isOwner: message.senderId === socket.id,
      }])
    })

    socket.on(events.retrieveUser, (oldUser) => retrieveUser(oldUser))
  }, [socket])

  // return our application
  return (

    userRole === 'server'
      ? <ServerView socket={socket} users={users} userRole={userRole} />
      : (isRegistered
        ? (
          <UserView
            setUserRole={setUserRole}
            socket={socket}
            isConnected={isConnected}
            users={users}
            messages={messages}
            hiddenWord={hiddenWord}
            userDrawer={userDrawer}
            setRegistered={setRegistered}
          />
        )

        : <HomePage socket={socket} setRegistered={setRegistered} />
      )
  )
}
export default App
